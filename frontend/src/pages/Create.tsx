...
const Create: React.FC = () => {
    ...
    const handleMediaChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.files && event.target.files.length > 0) {
            ...
            if (files.length > 3) {
                console.error("Please upload a maximum of 3 images.");
    ...
    const handleMediaChangePost = (event: React.ChangeEvent<HTMLInputElement>): void => {
        if (event.target.files && event.target.files.length > 0) {
    ...
    const onSubmitStory = async (data: any) => {
        await uploadStory(data.storyCaption, selectedFile ? selectedFile.map(item => item.file) : [], data.isChecked);
                        ...
                        <div className="bg-white p-8 rounded-lg w-full max-w-md dark:bg-black dark:text-white">
                            <form onSubmit={handleSubmit(onSubmitPost)} className="space-y-6">
                                <div>
                                    ...
                                    <textarea id="postdescription" {...register("postdescription")} rows={5} className="mt-1 block w-full px-3 py-2 dark:bg-black border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Enter your description"></textarea>
                                </div>
                                {selectedMediaPost ? (
